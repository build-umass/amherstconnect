...
import type { Event, EventCategory,UserLocation,MapSectionProps,EventPreviewCardProps,NearbyPanelProps } from '../../types/event';
...
// MapSection combines the map + search bar + filter chips
// Search and chips are absolutely positioned over the map
  ...
  return (
    <View style={styles.mapContainer}>
      ...
      >
        {events.map(event => (
          ...
          >
            <View style={[
              styles.pin,
              { borderColor: CATEGORY_COLOR[event.category] },
      ...
      <View style={styles.mapSearchBar}>
        ...
        <Text style={styles.mapSearchPlaceholder}>Search campus events...</Text>
      </View>

      {/* Filter chips float below search bar */}
      <ScrollView
        horizontal
        showsHorizontalScrollIndicator={false}
        style={styles.mapFilterBar}
        contentContainerStyle={styles.mapFilterContent}
      >
        {filters.map(f => (
          <TouchableOpacity
            key={f}
            style={[styles.mapChip, active === f && styles.mapChipActive]}
            onPress={() => setActive(f)}
          >
            <Text style={[styles.mapChipText, active === f && styles.mapChipTextActive]}>
              {f}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      >
        {events.map(event => (
          ...
          >
            ...
            <View style={styles.nearbyCardImg}>
              <View style={[
                styles.nearbyCardBadge,
                { backgroundColor: CATEGORY_COLOR[event.category] + '25' },
              ]}>
                <Text style={[
                  styles.nearbyCardBadgeText,
                  { color: CATEGORY_COLOR[event.category] },
                ]}>
                  {event.category}
                </Text>
...
const styles = StyleSheet.create({
  ...
  headerGear:   { padding: 4 },
  gearIcon:     { fontSize: 20, color: '#666' },

  // Map container — relative so search/chips can float over it
  ...
  mapSearchIcon:        { fontSize: 14 },
  mapSearchPlaceholder: { fontSize: 14, color: '#999' },

  // Filter chips — floats over map below search bar
  mapFilterBar:    { position: 'absolute', top: 64, left: 0, right: 0 },
  mapFilterContent:{ paddingHorizontal: 16, gap: 8 },
  mapChip:         { paddingHorizontal: 16, paddingVertical: 7, borderRadius: 20, backgroundColor: '#fff', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 2 },
  mapChipActive:   { backgroundColor: MAROON },
  mapChipText:     { fontSize: 13, fontWeight: '500', color: '#444' },
  mapChipTextActive: { color: '#fff' },

  // Pins — white circle with category-colored border
  pin:          { width: 40, height: 40, borderRadius: 20, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center', borderWidth: 2.5, borderColor: MAROON, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.15, shadowRadius: 3, elevation: 4 },
  pinSelected:  { width: 50, height: 50, borderRadius: 25, borderWidth: 3 },
  ...
  nearbyCard:          { width: 155, borderRadius: 14, overflow: 'hidden', borderWidth: 1, borderColor: '#ebebeb', backgroundColor: '#fff' },
  nearbyCardImg:       { height: 90, backgroundColor: '#f5f5f5', justifyContent: 'space-between', padding: 10 },
  nearbyCardBadge:     { alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6 },
  nearbyCardBadgeText: { fontSize: 11, fontWeight: '700', textTransform: 'capitalize' },
  walkBadge:           { alignSelf: 'flex-end', backgroundColor: 'rgba(0,0,0,0.55)', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 3 },
  ...
  nearbyCardTime:      { fontSize: 11, color: MAROON, fontWeight: '600', marginBottom: 2 },
